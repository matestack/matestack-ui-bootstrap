class Bootstrap::Form::Submit < Matestack::Ui::Component
  ...
  def response
    form_submit do
      btn type: "submit", variant: button_variant || :primary, attributes: { "v-if": "!loading()" } do
        plain text || "Submit"
      end
      btn type: "submit", variant: button_variant || :primary, attributes: { disabled: true, "v-if": "loading()" } do
